...
void step1() {
    ...
    for (unsigned int i = 0; i < arrSize; i++) {
        ...
        unsigned long int range = (UINTMAX / numBuckets) + 1;
        if (numBuckets == 1)
...
void step2() {
    // TODO: Sort each bucket (note, use sortOneVector to sort your bucket.  It's straightforward.)
      ...
      unsigned int threadLocalBucket{ 0 };

      while (true) {
          myMutex.lock();
          threadLocalBucket = currentBucket;
          currentBucket++;
          myMutex.unlock();

          if (threadLocalBucket >= numBuckets)
              break;

          sortOneVector(buckets[threadLocalBucket]);
